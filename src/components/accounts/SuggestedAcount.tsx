...
const SuggestedAcount = forwardRef(({ image, nickname, tick, lastname, firstname, likeCount, follwers, isFollow }: SuggestedAcountProps, ref: ForwardedRef<any>) => {
    const { darkMode, fullScreenMode } = useSelector((state: RootState) => state.app);
    const navigate = useNavigate();
    return (
        <div onClick={() => navigate(`/@${nickname}`)}>
            <Tippy
                visible={fullScreenMode ? false : undefined}
                interactive
                delay={[800, 0]}
                offset={[0, 10]}
                ...
                render={(attrs) => (
                    <div {...attrs} className="w-[320px] bg-white h-[171px] p-5 shadow-primary rounded-lg">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            {!isFollow ? (
                                <Button className={`${darkMode ? 'bg-[rgba(255,255,255,0.08)]' : ''}  p-4 flex items-center justify-center bg-primary rounded-md font-semibold`}>
                                    <span className="text-white">Follow</span>
                                </Button>
                            ) : (
                                <Button
                                    className={`${darkMode ? 'bg-[rgba(255,255,255,0.08)]' : ''}  p-4 flex items-center justify-center rounded-sm hover:bg-[rgba(22,24,35,0.03)]`}
                                >
